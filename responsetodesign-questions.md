...
the user define the product order , by dragging and dropping  

a campaign is defined 

the system must 



for example : at 10:00 we send a message with the  the ai suggest a series of following sequences based on the if the ai is wrong , the user give feedback to the ai to adjust or discard completely the ai but every user interaction 
...
## RESPONSE 2 : a combination of 

### Option E: Enterprise franchise
A parent Organization with sub-Organizations (franchise locations). Some products are shared across sub-Organizations (global catalog), others are local. Campaigns can be pushed from parent to children.


but we will start as option C to make the implementation easy you must struture the database for more scalable use now 
...
### Response to option C : maybe we will have parsing rules but do not assume that parsing rules , regex , pattern recogintion with or without ai ai must be used only when non ai technique fail to reduce cost 
...
### response to option D : for this system the supplier is not 
...
##RESPONSE 3 : the goal of the extraction is to filter out junk data or bad formatted  so that they are pending before that means if the data are correct  , the human verify if the data is good , if the image display coorectly and can decide to put soe product in a pending approval state before they are usable in the system the user can toogle ai button so that the ai help by highlighting even before the ai we must have a prevalidator preconfigured system that highlight probabilistic fields for each product  in red  important the system must keep track of every human action and why the action so that the ai learn data processing rules  for specific task so the response is a combination of every option 
...
## RESPONSE 4 : a combination of both with coherence 
...
### Response 5: to adress the number of post per day what determines the : the availability of a human agent to post manually,  that means the human agent that have the role to post must define some where it's working hours so that the scheduling system  the number of group to post to , the number of number that will be used as sender , the goal is to maximize post , while following the strategy without getting ban that are the constrains 
...
### Response 6 : to adress this question you must first understand our goal and the constraints the goal is to have constant sending activity in the group for example they group participants must that is why we can change product media , product description , product sending order  participants boredom , whatsapp anti spam pattern we must fly under the meta radars ,and make participants think we have unlimited product when in reality  we don't we are just clever so it is a combination of every option + others thing 
...
### response option B : good idea fbut for the moment we will keep native walink to avoid friction because with native wa.me link , the sending process is faster because the customer are not redirected out of whatsapp before it comes back it dirrectly open the target numnber with the prefilled message it will be a good idea to track link clink but for now we want simplicity maybe we will add that later so design the software feature in a way that jut the personalized  native wa.me link with variable that are target number , message template maybe we will use a complementary software to count the number of specific message initiated for each product 
...
### Response C : one link click doesn't means that the person who clicked bought 
...
### Response D : good idea but for now we will keep it simple maybe we will add that later , so design in a w, we will use maybe keyword , regex , or product code ddetetion that means the prefilled message strucuture must be exportable to use in the chatbot builder or automation builder to make the creation of automated reply based on specific product easier to avoid manual process 
...
## response  8 : option B both manual and api first from day one also export 
...
## Response 9 : every option mentionned i can add that we need the software before we post both api or manually and after as an archive for 2 reasons remember what we posted exactly and track performance of human agent in charge of manual posting to compare multiple agents performance and reward them accordingly if  a human agent doesn't mark  a post as done the system must note and ask if it have an incovenience and notice the real post time to recalcualte the sendiong process 
...
##Response 10 : everything plus additionnal function we will also have the functioonality to turn a list of text only product and visualize images search result for each product and to have the right product images and save a corresponding media as a feature 
